...
namespace ProjectDelta.Tests.EditMode // EditMode 테스트 네임스페이스
{
    public sealed class StatusEffectApplicationServiceTests // 63일차 상태 규칙 테스트
    {
        ...
        public void TryApply_FailedRoll_DoesNotAddStatus() // 실패 시 미적용 검증
        {
            ...
            StatusEffectApplyResult result = StatusEffectApplicationService.TryApply(target, "SE001", "MON_A", 2, -3, StatusStackRule.Stack, 3, 95, 0, 0, random); // 95% 상태 적용 시도
        ...
        public void TryApply_NewStatus_AddsOneStack() // 최초 부여 검증
        {
            ...
            StatusEffectApplyResult result = StatusEffectApplicationService.TryApply(target, "SE001", "MON_A", 2, -3, StatusStackRule.Stack, 3, 80, 0, 0, random); // 중독 최초 적용
        ...
        {
            BattleParticipant target = CreateTarget(0); // 저항 0 대상 생성
            target.AddStatusEffect(new StatusEffectInstance("SE003", "MON_A", 1, 1, 0)); // 약화 기존 상태 추가
            FixedRandomSource random = new FixedRandomSource(1); // 성공 굴림 고정
            StatusEffectApplicationService.TryApply(target, "SE003", "MON_A", 3, 0, StatusStackRule.RefreshDuration, 1, 80, 0, 0, random); // 약화 재부여
        ...
        {
            BattleParticipant target = CreateTarget(0); // 저항 0 대상 생성
            target.AddStatusEffect(new StatusEffectInstance("SE001", "MON_A", 1, 2, -3)); // 중독 2중첩 상태 추가
            FixedRandomSource random = new FixedRandomSource(1); // 성공 굴림 고정
            StatusEffectApplicationService.TryApply(target, "SE001", "MON_A", 2, -3, StatusStackRule.Stack, 3, 80, 0, 0, random); // 3중첩 적용
            StatusEffectApplicationService.TryApply(target, "SE001", "MON_A", 4, -3, StatusStackRule.Stack, 3, 80, 0, 0, random); // 상한 이후 재부여
        ...
        {
            BattleParticipant target = CreateTarget(0); // 저항 0 대상 생성
            target.AddStatusEffect(new StatusEffectInstance("SE005", "MON_A", 1, 1, 0)); // 기절 기존 상태 추가
            FixedRandomSource random = new FixedRandomSource(1); // 성공 굴림 고정
            StatusEffectApplicationService.TryApply(target, "SE005", "MON_A", 3, 0, StatusStackRule.NoStack, 1, 80, 0, 0, random); // 기절 재부여 시도
            Assert.AreEqual(1, target.StatusEffects.Count); // 상태 개수 유지 확인
            Assert.AreEqual(1, target.StatusEffects[0].StackCount); // 중첩 증가 없음 확인
        ...
        public void TryApply_UsesTargetResistanceInChance() // 대상 저항 반영 검증
        {
            ...
            StatusEffectApplyResult result = StatusEffectApplicationService.TryApply(target, "SE003", "MON_A", 2, 0, StatusStackRule.RefreshDuration, 1, 70, 0, 0, random); // 최종 40% 상태 적용 시도
