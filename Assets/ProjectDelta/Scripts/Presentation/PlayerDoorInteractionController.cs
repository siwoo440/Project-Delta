...
namespace ProjectDelta.Presentation // 프레젠테이션 네임스페이스
{
    public sealed class PlayerDoorInteractionController : MonoBehaviour // 플레이어 문 상호작용 제어
    {
        ...
        [SerializeField] private RoomPassageController passageController; // 현재 방 통로 컨트롤러
        ...
        private void Awake() // 상호작용 참조 자동 연결
        {
            ...
            {
                passageController = FindFirstObjectByType<RoomPassageController>(); // 현재 방 통로 컨트롤러 검색
            }
        }
        ...
        private void OnInteract(InputAction.CallbackContext context) // F 상호작용 처리
        {
            ...
            if (playerState == null || passageController == null) // 필요한 런타임 상태 확인
            {
                return; // 상호작용 중단
            }

            CardinalDirection facing = GetFacingDirection(); // 현재 바라보는 4방향 계산
            DoorOpenResult result = passageController.TryOpenDoor(playerState.CurrentGridPosition, facing, playerState); // 바로 앞 문 열기 시도
        ...
        private string BuildPromptText() // 정면 문 안내 문구 생성
        {
            ...
            if (!passageController.TryGetDoor(playerState.CurrentGridPosition, facing, out GridPassage doorPassage)) // 바로 앞 문 존재 확인
            {
